...
## 1. 先给结论：算法相同，kernel 边界不同

FLA 和 FlashInfer 的基础路径都精确实现 Gated Delta Rule。它们先把逐 token recurrence 等价改写为 chunk 公式，在 chunk 内用矩阵乘和下三角求解并行计算，在 chunk 间继续传递 recurrent state；两者都没有截断历史。

对一个 chunk，它们计算同样的四个量：
...
$$
V_{new}=A\operatorname{diag}(\beta)
\left(V-\operatorname{diag}(e^\gamma)KS_c\right),
$$

$$
S_{c+1}=e^{\gamma_C}S_c+
\left[\operatorname{diag}(e^{\gamma_C-\gamma})K\right]^\top V_{new},
$$

$$
O_c=s\left[
\operatorname{diag}(e^\gamma)QS_c+
...
```text
log gate / alpha -> gamma, G --------------------+
K, beta, G -> K K^T -> triangular solve A ------+
K, V, S_c, A -> residual writes V_new ----------+-> S_(c+1)
Q, K, V_new, S_c --------------------------------+-> O_c
```

真正的差别是这些箭头在哪个 kernel 中执行、哪些中间量写入 HBM，以及并行度不足时如何拆分 chunk 间状态链。
...
## 4. FlashInfer：把同一组公式压入持久 kernel
...
### 4.3 Blackwell warp specialization
