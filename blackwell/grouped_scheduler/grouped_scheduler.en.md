...
> Source: <https://docs.nvidia.com/cutlass/latest/media/docs/cpp/grouped_scheduler.html>

![GEMM hierarchy with epilogue](images/gemm-hierarchy-with-epilogue-no-labels.png)
...
![Grouped GEMM schedule for equal-sized 2x2 problems](images/grouped-gemm-schedule-2x2.png)

A similar mapping for problems that do not have the same number of tiles is shown below:

![Grouped GEMM schedule for varied problem sizes](images/grouped-gemm-schedule-varied.png)
...
![Grouped SYR2K schedule using the grouped GEMM scheduler](images/grouped-syr2k-schedule-using-grouped-gemm-scheduler.png)
...
![Ideal grouped SYR2K schedule](images/grouped-syr2k-schedule-ideal.png)

Achieving this schedule requires mapping from a threadblock ID to tile coordinates `(i, j)`.

We will illustrate this by mapping a lower-triangular matrix with a 3x3 grid. We first calculate row and column indices assuming one-indexed rows, tiles, and threadblock IDs, and then subtract one to convert to zero-indexed versions. Our description borrows heavily from the mapping described [here](https://stackoverflow.com/a/40954159).

![Grouped SYR2K schedule for a 3x3 grid](images/grouped-syr2k-schedule-3x3.png)
...
The closed-form equation for the right-hand side is: `i(i+1)/2`. Using this, we can solve for `i` given `t`:
...
Threadblock IDs within a given row are sequential, so the one-indexed column ID for one-indexed threadblock ID `t` and row `i` is:
...
![Grouped SYR2K schedule with macro tiles](images/grouped-syr2k-schedule-macro.png)
...
Where `r` is the ratio of the maximum dimension of the grid to the minimum dimension of the grid (i.e., `r = 4 / 2 = 2` in the previous example).
...
The grouped kernel schedulers come with two different modes for finding the next tile for a block to compute. These techniques are controlled by the [`cutlass::gemm::kernel::GroupScheduleMode`](https://github.com/NVIDIA/cutlass/tree/main/include/cutlass/gemm/kernel/grouped_problem_visitor.h) enum. We describe each mode in greater detail below.
...
To ease the process of sorting groups and their associated metadata in this manner, the device-level grouped kernels provide a `sort_problems()` method. An example of how to use this may be found in the [grouped GEMM example](https://github.com/NVIDIA/cutlass/tree/main/examples/24_gemm_grouped/gemm_grouped.cu).
