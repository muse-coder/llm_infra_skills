...
> **Subheading:** Microbenchmarking, tcgen05, 2SM MMA, UMMA, TMA, LDGSTS, UBLKCP, Speed of Light, Distributed Shared Memory, GPC Floorsweeps, SM Yield
>
> Authors: [Kimbo Chen](https://substack.com/@kimbobachen) and [Dylan Patel](https://substack.com/@semianalysis) · Mar 31, 2026 · **Paid**
>
> Source: <https://newsletter.semianalysis.com/p/dissecting-nvidia-blackwell-tensor>
>
> ⚠️ **This is a paid SemiAnalysis post. Only the free preview portion could be retrieved.** The article continues past the "Tensor Core throughput / latency / in-flight" section into the paywalled content (real-world CUTLASS use cases, floorplans, PTX/SASS deep-dive, floorsweep & yield). The paywalled portion is **not** included below.

Nvidia's Datacenter Blackwell GPU (SM100) represents one of the largest GPU microarchitecture changes in a generation, yet no detailed whitepaper exists. Until today, there is no public datacenter Blackwell architecture microbenchmarking study on PTX and SASS instructions, such as UMMA and TMA, with a focus on AI workloads.

After our in-depth [Nvidia Tensor Core Evolution: From Volta To Blackwell article](https://newsletter.semianalysis.com/p/nvidia-tensor-core-evolution-from-volta-to-blackwell), SemiAnalysis has spent months of engineering time tearing into the Blackwell architecture and measuring the raw PTX instruction performance, to establish hard practical performance upper bounds and compare them with the theoretical peaks. We do this to discover unit- and instruction-level hardware throughput and latency limits, providing a useful characterization from an ML systems and kernel development perspective. We focus on deep learning workload configurations, such as benchmarking asynchronous memory copy setups used in the popular deep learning library FlashInfer.
...
We thank Nebius and Verda for providing B200 nodes for microbenchmarking. Their B200 nodes have the correct hardware counters enabled that make NCU profiling possible. For users on cloud providers that don't have NCU enabled, [here is a workaround](https://x.com/marksaroufim/status/2018739807363674373) suggested by GPU Mode's Mark Saroufim. We would also like to thank the authors of [Dissecting the NVIDIA Hopper Architecture through Microbenchmarking and Multiple Level Analysis](https://github.com/HPMLL/NVIDIA-Hopper-Benchmark) and [tcgen05 for dummies](https://github.com/gau-nernst/learn-cuda/tree/main/02e_matmul_sm100), whose work we based our code upon.
...
Join us if you want to work on low level benchmarking, ClusterMAX, inference simulators, or other interesting technical work. Send your resume to letsgo@semianalysis.com with 5 bullet points demonstrating your exceptional engineering abilities.
...
* The introduction of **tensor memory (TMEM)** to hold MMA accumulators. Threads no longer implicitly own the results of MMA operations; instead, TMEM is explicitly managed at the MMA scope from software.
...
* [Cluster Launch Control (CLC)](https://docs.nvidia.com/cutlass/latest/media/docs/cpp/blackwell_cluster_launch_control.html) as hardware support for dynamic work scheduling in persistent-CTA kernels (covered in future articles).
* [Programmatic dependent launch (PDL)](https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/programmatic-dependent-launch.html) was introduced in Hopper to hide launch and setup latency in back-to-back kernels (covered in future articles).
...
![Logical grouping of TPCs into GPCs (floorsweep result)](images/fig01_gpc_floorsweep.png)
...
![SM-to-SM L2 latency distance matrix](images/fig02_logical_vs_physical_gpc.png)
...
![Latency profile for a single SM](images/fig03_gpc_layout.png)
...
![Async copy throughput vs bytes-in-flight (MHA config)](images/fig04_async_copy.png)

Overall, we see memory throughput with `LDGSTS` saturating at around 6.6 TB/s at 32 KiB in flight.

We also benchmark the config space multi-latent attention (MLA) kernels use:
...
![Async copy throughput (MLA config)](images/fig05_ldgsts.png)
...
![Async copy latency](images/fig06_async_copy2.png)

![Async copy summary table](images/fig07_async_copy_table.png)
...
* TMA box dimensions: 2D shapes increasing from size 32×8 to 128×128

![TMA throughput vs bytes-in-flight](images/fig08_tma.png)
...
![Async copy vs TMA — throughput](images/fig09_tma_vs_async1.png)

![Async copy vs TMA — latency](images/fig10_tma_vs_async2.png)
...
![TMA multicast: explicit vs implicit](images/fig11_tma_multicast.png)
...
We experimented with a few different PTX patterns for interacting with DSMEM. An important difference when writing code for DSMEM vs. SMEM is that DSMEM loads are packetized similar to global loads, so the optimal access pattern looks nothing like the interleaved accesses that avoid bank conflicts in local SMEM, and more like a typical coalesced access to contiguous locations in GMEM. Additionally, we observed that in order to get the full 128 B/cycle for local SMEM, `ld.shared` must be used without `::cluster`. This was a pitfall we ran into when we wrote a benchmark which simply used `ld.shared::cluster` to local and remote SMEM addresses. With `ld.shared`, the compiler emits `LDS` instead of a generic `LD` emitted with `ld.shared::cluster`, which does not seem to be able to achieve peak throughput for local SMEM. We also struggled to push the achieved throughput further with `ld.shared::cluster`, and only achieved slightly higher throughput through DSMEM after switching to `cp.async.bulk` (PTX) / `UBLKCP` (SASS) to move a higher volume of data per instruction.
...
![DSMEM vs SMEM peak throughput per PTX pattern](images/fig12_dsmem_vs_smem.png)
...
We benchmarked MMA performance with the configuration space below:

![MMA benchmark configuration space](images/fig13_tcgen05_mma.png)
...
![UMMA throughput vs theoretical peak](images/fig14_throughput1.png)
...
![Throughput across M/N sizes](images/fig15_throughput2.png)
...
![SS vs TS layout throughput](images/fig16_throughput3.png)
...
![SMEM-bound vs math-bound crossover table](images/fig17_throughput4.png)

The same is true for other datatypes — MMA instructions with both operands in SMEM are SMEM-bound below N=128.

To further illustrate the point, we plot the roofline for all shapes of FP8 1SM MMA. We see clearly that N < 256 is in the memory-bounded region, and the slope is roughly 128 bytes / cycle, the SMEM bandwidth.

![FP8 1SM MMA roofline](images/fig18_throughput5.png)

2SM MMA achieves perfect weak scaling across all formats and shapes, reaching 2× speedup when using 2× the amount of compute resources than 1SM MMA. In smaller shapes of ABLayout=SS, we observe over 2× speedup, which again happens because the instruction is SMEM bound below N=128 for SS and the 2SM version splits operand B between the two SMs.

![2SM weak scaling — SS mode (>2x for N<128)](images/fig19_throughput6.png)

*SS mode: Over 2× speedup for N < 128 due to being SMEM bound*

![2SM weak scaling — TS mode (near-perfect 2x)](images/fig20_throughput7.png)

*TS mode: Near-perfect 2× speedup*
...
![MMA latency across shapes](images/fig21_latency1.png)

We notice a small but consistent pattern in the order of latency:
...
![MMA latency by data type](images/fig22_latency2.png)
...
Across all configurations, we see the same N and in-flight MMAs achieve similar percentages of Speed-of-Light (SoL). Notably, only the largest N reaches 90% SoL, while the smallest N achieves only about 70%. Comparing 1SM and 2SM MMA, we see 1SM achieves around 5% higher SoL throughput than its 2SM counterpart. For the same data format and CTA group MMA, the throughput for larger N is always higher than smaller N sizes. Finally, we observe that the throughput SoL percentages for 4 in-flight MMAs cap out at 78%–80%.

![Throughput vs in-flight instruction count (1)](images/fig23_inflight1.png)

![Throughput vs in-flight instruction count (2)](images/fig24_inflight2.png)

![Throughput vs in-flight instruction count (3)](images/fig25_inflight3.png)

---

> **[Paywall]** The remainder of the article ("real-world use cases with the kernel-writing library CUTLASS … throughput, multicast, and floorplans", plus the PTX/SASS deep-dive, floorsweep and SM yield analysis) is for paid subscribers and was not retrievable.
>
> Read the full article: <https://newsletter.semianalysis.com/p/dissecting-nvidia-blackwell-tensor>
