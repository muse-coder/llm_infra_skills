...
>
> Posted May 9, 2026 · Colfax Research · Article / Blog / Tutorials
...
where $A \in \mathbb{R}^{M\times K}$, $B \in \mathbb{R}^{K\times N}$, and $C \in \mathbb{R}^{M\times N}$. The computation of C is parallelized by dividing the problem shape (M, N, K) by some tile shape (bM, bN, bK), and computing each bM x bN output tile as
...
![The output C of a GEMM is partitioned into a 5 x 6 grid of work tiles, each computed by one of eight clusters; the tiles assigned to cluster 0 are highlighted](images/fig01_static_persistent_partition.png)
...
![Each work tile in the grouped GEMM is assigned to one of eight clusters by linearly ordering all tiles and assigning every 8th tile to a cluster](images/fig02_static_assignment.png)
...
![The FLOPs computed by each cluster in the static persistent case, showing significant imbalance](images/fig03_static_flops_imbalance.png)
...
![In the dynamic persistent case, each cluster gets an initial tile then fetches new tiles as it completes work](images/fig04_dynamic_assignment.png)
...
![The FLOPs computed by each cluster in the dynamic persistent case, showing a more uniform distribution](images/fig05_dynamic_flops.png)
...
![Performance of a highly load-imbalanced grouped GEMM with static and dynamic scheduling; dynamic significantly outperforms static as imbalance grows](images/fig06_static_vs_dynamic_perf.png)
...
* `.is_canceled` gives a predicate indicating whether the requested cancellation was successful. Note that if `.is_canceled` returns false then `query_cancel` instructions other than `.is_canceled` result in undefined behavior, so we should always start with `.is_canceled`.
  * Note further that if a CTA has observed the failure of a `try_cancel` (i.e., `.is_canceled` returns false) then issuing another `try_cancel` also results in undefined behavior. Thus, after such an observance, the CTA can no longer use CLC and should exit after exhausting its current work queue.
...
The Blackwell CuTeDSL example [dense_gemm_persistent_dynamic.py](https://github.com/NVIDIA/cutlass/blob/main/examples/python/CuTeDSL/cute/blackwell/kernel/dense_gemm/dense_gemm_persistent_dynamic.py) implements a standard dense GEMM with CLC `try_cancel` performed by a single scheduler warp in each cluster, and communication between this and other warps in the cluster handled by a CLC pipeline. The numbering of the warps per CTA launched by the kernel can be seen in the `__init__` method:
...
What are `is_signalling_thread` and `producer_mask`? The answer can be found earlier in the class:
...
Line 8 is where the response data is decoded (`clc_response` boils down to operations corresponding to the PTX instructions `clusterlaunchcontrol.query_cancel`). Since the CTA coordinates *in the grid* obtained from `query_cancel` are always the first in the cluster, we offset by this CTA's coordinates *in its cluster* to properly get its tile coordinates.
...
![CLC vs static persistent vs single-tile scheduling on balanced GEMMs, K = 2048](images/fig07_balanced_perf_1.png)

![CLC vs static persistent vs single-tile scheduling on balanced GEMMs, K = 8192](images/fig08_balanced_perf_2.png)
...
![Tensor pipe throughput timeline for CLC, problem shape (16384, 16384, 2048)](images/fig09_clc_tensorpipe_16384.png)

And for static persistent:

![Tensor pipe throughput timeline for static persistent, problem shape (16384, 16384, 2048), showing a gradual dropoff](images/fig10_static_tensorpipe_16384.png)
...
![Tensor pipe throughput timeline for CLC, problem shape (32768, 32768, 2048), showing consistently lower utilization](images/fig11_clc_tensorpipe_32768.png)

And for static persistent:

![Tensor pipe throughput timeline for static persistent, problem shape (32768, 32768, 2048)](images/fig12_static_tensorpipe_32768.png)

So in this case somehow the dropoff is less severe with a static scheduler, while the tensor pipe throughput appears to be consistently lower with CLC. One metric that correlates with this observation with (32768, 32768, 2048) is that NCU reports L2 hit rate to be only 35% with CLC compared to 52% with static persistent. The reason for this difference is unclear. Note that neither kernel has work tile swizzling, and with problem shape (16384, 16384, 2048), NCU shows an L2 hit rate of about 60% for both kernels.
...
![Histogram of tiles computed per SM pair with CLC, problem shape (16384, 16384, 2048)](images/fig13_tiles_histogram_16384.png)

With a problem shape of (32768, 32768, 2048), the histogram of tiles computed instead looks like this:

![Histogram of tiles computed per SM pair with CLC, problem shape (32768, 32768, 2048)](images/fig14_tiles_histogram_32768.png)
