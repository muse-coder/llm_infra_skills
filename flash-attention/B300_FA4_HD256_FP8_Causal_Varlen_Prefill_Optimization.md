...
## 2. 整体优化思路：先解决全局 pipeline，再下钻局部指令

最终方案不是若干独立 trick 的集合，而是围绕同一目标组织：让一个 M256 work 的数据搬运、Tensor Core、SIMT softmax、O correction 和输出尽量连续，同时让 B300 上的 resident clusters 始终拿到足够且负载均衡的 works。

整体分成四个大优化方向。

### 2.1 大方向一：先建立无明显 bubble 的主流水线

```text
GMEM/L2          Q once        K/V(i+2...) TMA prefetch
                  │                    │
SMEM              Q            5-stage K/V ring
                  │                    │
Tensor Core       ├─ QK(i+1) ──────────┤
                  │                    │
TMEM/register     └─ S(i) → softmax → FP8 P(i)
                                      │
Tensor Core                          PV(i) → O
                                                │
TMEM/register                              correction(i-1)
                                                │
SMEM/register                              vector epilogue → GMEM
```

主流水线的关键不是让某一个阶段单独最快，而是缩短端到端 critical path：

- 5-stage K/V TMA ring 隐藏 GMEM/L2 latency；
- 两个 S/P TMEM slots 让 `QK(i+1)` 与 `softmax/P/PV(i)` 交错；
- `O_full → correction → O_rescaled` 独立成第三条依赖链；
- split-P 在 P 写到 75% 时提前启动 PV；
- epilogue 复用 correction warps，避免单独增加常驻 warp。

后续首先完整解释这条 pipeline，再解释每条指令和每个 buffer。

### 2.2 大方向二：再设计 work geometry 和 persistent 调度

算子即使单个 work 很快，如果 work 数跨过 wave cliff，或者 causal 重 work 留在 grid 尾部，整 kernel 仍会很慢。最终调度分四层：

```text
PackGQA   减少 M 方向 tail padding 和 logical work 数
    ↓
wave      让 work 数尽量贴合 B300 的 74-cluster wave
    ↓
LPT       先发 causal K-range 更长的重 works
    ↓
CLC       resident cluster 动态 steal 尚未启动的 work
```

CLC 让 physical cluster 跨 work 存活，因此 KPP slot/phase、Q/KV ring state 和 O pipeline generation 也必须跨 work 延续。调度和流水线不是两套互不相关的优化；persistent lifetime 直接决定 barrier 状态机怎样写。

### 2.3 大方向三：把流水线映射到 B300 专用硬件

| 需求 | B300/SM103 feature | 最终用法 |
| --- | --- | --- |
| 完整 D256 QK/PV | 2CTA `tcgen05.mma` | `UTCQMMA.2CTA`，CTA pair 共同完成 reduction |
| 大块异步输入 | Tensor Memory Accelerator | Q 使用 `UTMALDG.3D.2CTA`；paged K/V 使用 page-sized `UTMALDG.4D` |
| Tensor Core accumulator | Tensor Memory | `S/P slot0 + S/P slot1 + O = 512 columns` |
| 计算/搬运并行 | warp specialization | 4 softmax + 4 correction/epilogue + 1 MMA + 1 load/CLC |
| 动态寄存器分配 | CTA register pool | softmax/correction/other=`160/128/96`，SASS 为 `USETMAXREG.*.CTAPOOL` |
| 动态 work stealing | Cluster Launch Control | leader CTA 的 load warp 异步预取 16-byte response |

硬件 feature 只有放进正确的 pipeline 才有价值。例如 TMA 本身不会自动带来收益；必须同时有足够 stage distance、正确的 transaction-byte barrier 和不会被提前覆盖的 SMEM ring。

### 2.4 大方向四：最后优化 bookkeeping、数值和验证闭环

主 pipeline 与调度稳定后，再处理局部但高频的成本：

- page16/32/64 的一个 KV128 tile 聚合多条 page-sized TMA；
- K/V 复用 register-local page-ID tuple，减少 page-table loads；
- correction/epilogue 按 16 columns 流式处理，避免 register spill；
- FP8 P scaling、online-softmax threshold 与 O correction 使用一致 scale；
- 用 SASS 确认 `UTCQMMA.2CTA/UTMALDG/USETMAXREG`，用 NCU 确认 tensor active、barrier、spill、bytes 和 wall time。

因此本文后续阅读顺序固定为：

```text
主流水线设计
    → work geometry / PackGQA / wave
    → LPT + CLC persistent 调度
    → B300 2CTA / warp / SMEM / TMEM / register 资源映射
    → TMA 与 Tensor Core 指令细节
    → 同步、epilogue、Paged-KV、FP8
    → 性能证据与验证方法
```

## 3. 主流水线：TMA、QK、softmax、PV、correction 的 overlap

### 3.1 三个彼此独立的 stage 维度
...
## 4. 调度设计（一）：PackGQA、physical cluster 与 wave

### 4.1 先区分 work count、physical cluster 与 wave
...
## 5. 调度设计（二）：causal LPT + persistent CLC

### 5.1 Scheduler work tuple 与 K-block 内循环

第 4 章先从 work 数和 wave 几何解释 PackGQA；这里进一步定义 scheduler 交给 persistent cluster 的单个 work 内部究竟包含什么。
...
### 5.8 为什么 odd K-block work 会破坏 KPP epoch
...
### 6.3 单个 logical K block 的计算量与片上搬运
...
## 7. 2CTA、warp specialization 与片上存储

### 7.1 2CTA 不是两个独立的 HD128 attention
...
### 7.3 用 warp-role register redistribution 消除 spill
...
### 8.2 Q：一次 work 只加载一次，64 KB / cluster
...
### 8.3 K/V：每个 K block 各 32 KB / cluster
...
### 8.4 page128/page256：一页包含一个或多个 logical tiles
...
### 8.5 page16/32/64：一个 logical tile 聚合多条 page-sized TMA
...
### 8.6 为什么 small page 的多 TMA 仍优于 LDGSTS gather
...
## 9. Tensor Core：QK 与 PV 都使用 `UTCQMMA.2CTA`
...
## 11. Epilogue：FP32 TMEM → BF16 GMEM

### 11.1 当前 epilogue 的完整路径
...
### 11.4 TMA store 与 TMA load 的同步不同
...
## 12. Paged-KV 寻址：减少 payload 之外的 load 指令

### 12.1 page、tile、physical page 是三个单位
...
### 12.2 K/V page ID register cache
...
### 12.3 masked tail 的物理 load 仍必须安全
...
## 13. FP8 数值路径也是 pipeline 设计

### 13.1 为什么 P 需要放大后再 cast FP8
